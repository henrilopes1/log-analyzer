...
from .utils import (
    setup_logging, load_data_file, parse_timestamp, 
    ensure_directory_exists, generate_timestamped_filename,
    validate_required_columns, clean_ip_address,
    format_duration, calculate_risk_score
)
...
class LogAnalyzer:
    ...
    def load_log_file(self, file_path: str, log_type: str = "unknown") -> Optional[pd.DataFrame]:
        """
        ...
        try:
            ...
            if 'timestamp' in df.columns:
                ...
                df = df.dropna(subset=['timestamp'])  # Remover registros com timestamp inválido
    ...
    def analyze_firewall_logs(self, df: pd.DataFrame) -> None:
        ...
        if denied_attempts.empty:
            self.console.print("[yellow]⚠️ Nenhuma tentativa bloqueada encontrada[/yellow]")
            return
        
        # Agrupar por IP de origem
        denied_by_ip = denied_attempts.groupby('source_ip').agg({
    ...
    def analyze_auth_logs(self, df: pd.DataFrame) -> None:
        ...
        # Agrupar por IP
        failed_by_ip = failed_attempts.groupby('source_ip').agg({
    ...
    def count_access_by_ip(self, df_firewall: Optional[pd.DataFrame], 
                          df_auth: Optional[pd.DataFrame]) -> None:
        """
        ...
        if not self.ip_access_count:
            self.console.print("[yellow]⚠️ Nenhum acesso encontrado para contabilizar[/yellow]")
            return
        ...
        self.analysis_stats['suspect_ips_found'] = sum(
            1 for count in self.ip_access_count.values() 
    ...
    def detect_brute_force(self, df: pd.DataFrame, 
        ...
        self.logger.info(f"Detectando brute force: {min_attempts}+ tentativas em {time_window} min")
        ...
        if failed_attempts.empty:
            self.console.print("[green]✅ Nenhuma tentativa falhada para análise[/green]")
            return
            ...
            for i in range(len(ip_attempts) - min_attempts + 1):
                ...
                if len(window_attempts) >= min_attempts:
                    ...
                    attack_info = {
                        ...
                        'duration': (window_attempts['timestamp'].max() - window_start).total_seconds(),
        ...
        if not brute_force_detected:
            self.console.print("[green]✅ Nenhum ataque de brute force detectado[/green]")
            return
    ...
    def detect_port_scanning(self, df: pd.DataFrame,
                           time_window_minutes: Optional[int] = None,
                           min_ports: Optional[int] = None) -> None:
        """
        ...
        self.logger.info(f"Detectando port scanning: {threshold}+ portas em {time_window} min")
        
        # Filtrar tentativas negadas (mais indicativo de scanning)
        denied_attempts = df[df['action'] == 'DENY'].copy()
        
        if denied_attempts.empty:
            self.console.print("[green]✅ Nenhuma tentativa negada para análise[/green]")
            return
        ...
        if not port_scan_detected:
            self.console.print("[green]✅ Nenhuma varredura de portas detectada[/green]")
            return
        ...
        for scan in sorted(port_scan_detected, key=lambda x: x['unique_ports'], reverse=True)[:3]:
            details = Text()
        ...
        try:
            ...
            for attack in self.brute_force_attempts:
                suspect_data.append({
                    ...
                    'primeira_deteccao': attack['start_time'].strftime('%Y-%m-%d %H:%M:%S'),
                    'ultima_deteccao': attack['end_time'].strftime('%Y-%m-%d %H:%M:%S'),
            ...
            for scan in self.port_scan_attempts:
                suspect_data.append({
                    ...
                    'primeira_deteccao': scan['start_time'].strftime('%Y-%m-%d %H:%M:%S'),
                    'ultima_deteccao': scan['end_time'].strftime('%Y-%m-%d %H:%M:%S'),
                ...
                if count >= config["high_threshold"]:
                    ...
                    if not any(item['ip'] == ip for item in suspect_data):
                        suspect_data.append({
                            ...
                            'ocorrencias': count,
                            'primeira_deteccao': datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
                            'ultima_deteccao': datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
                            'servicos_afetados': 'Múltiplos',
                            'usuarios_alvo': 'N/A'
                        })
            
            if not suspect_data:
                self.console.print("[yellow]⚠️ Nenhum IP suspeito encontrado para exportar[/yellow]")
                return
            
            # Escrever arquivo CSV
            with open(output_file, 'w', newline='', encoding='utf-8') as csvfile:
                fieldnames = ['ip', 'tipo_de_alerta', 'ocorrencias', 'primeira_deteccao', 
                             'ultima_deteccao', 'servicos_afetados', 'usuarios_alvo']
    ...
    def generate_summary(self) -> None:
        ...
        # Estatísticas gerais
        summary_text.append(f"📊 Total de logs processados: {self.analysis_stats['total_logs_processed']}\n", style="white")
        ...
        if self.ip_access_count:
            high_risk_count = sum(1 for count in self.ip_access_count.values() 
                                if count >= self.config["risk_classification"]["high_threshold"])
            if high_risk_count > 0:
                summary_text.append(f"• Monitore {high_risk_count} IPs de alto risco\n", style="white")
        ...
        if self.analysis_stats['analysis_start_time'] and self.analysis_stats['analysis_end_time']:
            duration = self.analysis_stats['analysis_end_time'] - self.analysis_stats['analysis_start_time']
    ...
    def _display_analysis_stats(self) -> None:
        ...
        stats_text.append(f"📊 Logs de firewall processados: {self.analysis_stats['firewall_logs']}\n", style="white")
        stats_text.append(f"🔐 Logs de autenticação processados: {self.analysis_stats['auth_logs']}\n", style="white")
        stats_text.append(f"🎯 IPs suspeitos encontrados: {self.analysis_stats['suspect_ips_found']}\n", style="yellow")
        stats_text.append(f"🌍 IPs analisados geograficamente: {len(self.ip_location_cache)}\n", style="blue")
        
        if self.analysis_stats['analysis_start_time'] and self.analysis_stats['analysis_end_time']:
            duration = self.analysis_stats['analysis_end_time'] - self.analysis_stats['analysis_start_time']
            stats_text.append(f"⏱️ Duração total: {duration.total_seconds():.2f} segundos\n", style="green")
