...
def run_command(cmd, description, allow_fail=False):
    ...
    try:
        ...
        if cmd.startswith("python "):
            python_path = (
                r"C:\Users\Henri\AppData\Local\Programs\Python\Python311\python.exe"
            )
        ...
        if result.returncode == 0:
            ...
            print(
                f"{'⚠️' if allow_fail else '❌'} {description} - {'Warning' if allow_fail else 'Failed'}"
            )
            if result.stderr:
...
def main():
    ...
    steps = [
        ...
        (
            "python -m black --check src/ || echo 'Format check done'",
            "Black format check",
            True,
        ),
        (
            "python -m flake8 src/ --max-line-length=88 --extend-ignore=E203,W503,F401 || echo 'Lint check done'",
            "Flake8 lint",
            True,
        ),
        # Segurança (allow_fail=True)
        (
            "python -m bandit -r src/ -ll || echo 'Security check done'",
            "Security scan",
            True,
        ),
        # Build (essencial)
