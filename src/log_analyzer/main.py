...
from .utils import validate_file_format, ensure_directory_exists
...
def validate_arguments(args):
    ...
    if not any([args.firewall, args.auth, args.samples, args.samples_json]):
        console.print("[red]❌ Erro: Especifique arquivos ou use --samples/--samples-json[/red]")
        return False
    
    if args.firewall and not os.path.exists(args.firewall):
        console.print(f"[red]❌ Arquivo de firewall não encontrado: {args.firewall}[/red]")
        return False
    
    if args.auth and not os.path.exists(args.auth):
        console.print(f"[red]❌ Arquivo de autenticação não encontrado: {args.auth}[/red]")
        return False
...
Exemplos de uso:
  ...
  %(prog)s --samples --brute-force-threshold 3 --port-scan-threshold 5
    ...
    input_group.add_argument('--firewall', type=str, 
                           help='Arquivo CSV/JSON com logs de firewall')
    input_group.add_argument('--auth', type=str, 
                           help='Arquivo CSV/JSON com logs de autenticação')
    input_group.add_argument('--samples', action='store_true', 
                           help='Usar arquivos de exemplo (CSV)')
    input_group.add_argument('--samples-json', action='store_true', 
                           help='Usar arquivos de exemplo (JSON)')
    
    # Parâmetros de detecção
    detection_group = parser.add_argument_group('Parâmetros de Detecção')
    detection_group.add_argument('--brute-force-threshold', type=int, default=5,
                               help='Número mínimo de tentativas para detectar brute force (padrão: 5)')
    detection_group.add_argument('--time-window', type=int, default=1,
                               help='Janela de tempo em minutos para detectar brute force (padrão: 1)')
    detection_group.add_argument('--port-scan-threshold', type=int, default=10,
                               help='Número mínimo de portas para detectar port scan (padrão: 10)')
    detection_group.add_argument('--port-scan-window', type=int, default=1,
                               help='Janela de tempo em minutos para detectar port scan (padrão: 1)')
    
    # Análise geográfica
    geo_group = parser.add_argument_group('Análise Geográfica')
    geo_group.add_argument('--disable-geo', action='store_true',
                         help='Desabilitar análise geográfica (útil para melhor performance)')
    geo_group.add_argument('--geo-timeout', type=int, default=5,
    ...
    export_group.add_argument('--export-csv', type=str, default=None,
    ...
    try:
        ...
        if df_firewall is None and df_auth is None:
            ...
            console.print("[yellow]💡 Verifique os caminhos dos arquivos e tente novamente[/yellow]")
            sys.exit(1)
        ...
        if not args.disable_geo and (df_firewall is not None or df_auth is not None):
            ...
            analyzer.analyze_geographic_patterns(pd.concat([df for df in [df_firewall, df_auth] if df is not None], ignore_index=True))
