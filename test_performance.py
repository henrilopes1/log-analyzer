...
class PerformanceProfiler:
    ...
    def record_metric(self, operation: str, duration: float, 
                     memory_used: float = None, cpu_percent: float = None):
...
def test_core_performance(profiler: PerformanceProfiler):
    ...
    for size in test_sizes:
        ...
        print(f"    ✅ {size:,} registros: Init={init_time*1000:.1f}ms, "
...
def test_geographic_performance(profiler: PerformanceProfiler):
    ...
    for batch_size in batch_sizes:
        ...
        print(f"    ✅ Lote {batch_size}: {duration*1000:.1f}ms ({len(results)} resultados)")
...
def test_concurrent_performance(profiler: PerformanceProfiler):
    ...
    for num_threads in thread_counts:
        ...
        with ThreadPoolExecutor(max_workers=num_threads) as executor:
            futures = [
                executor.submit(analyze_data, i, data_size) 
        ...
        print(f"    ✅ {num_threads} threads: Total={total_duration*1000:.1f}ms, "
...
def test_memory_usage(profiler: PerformanceProfiler):
    ...
    for size in data_sizes:
        ...
        print(f"    📈 {size:,} registros: {memory_used:.1f} MB "
