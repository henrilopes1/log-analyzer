...
def demo_core_functionality():
    ...
    try:
        ...
        print(
            f"   📊 Resultados: {len(brute_force_results)} tentativas suspeitas detectadas"
        )

        if len(brute_force_results) > 0:
            for idx, result in brute_force_results.iterrows():
                print(
                    f"   ⚠️  IP: {result['source_ip']} - {result['failed_attempts']} tentativas"
                )
        else:
        ...
        try:
            ...
            with open(temp_file, "rb") as file:
                ...
                response = requests.post(
                    f"{base_url}/analyze/", files=files, timeout=30
                )
                ...
                if response.status_code == 200:
                    ...
                    print(
                        f"   📊 Arquivos processados: {summary.get('files_processed', 0)}"
                    )
                    print(f"   📈 Total de eventos: {summary.get('total_events', 0)}")
                    print(
                        f"   ⏱️  Tempo de processamento: {summary.get('processing_time_seconds', 0):.2f}s"
                    )
                    ...
                    if brute_force:
                        print(
                            f"   ⚠️  Ataques de força bruta detectados: {len(brute_force)}"
                        )
                    else:
...
def demo_cache_system():
    ...
    try:
        ...
        if cached_data:
            ...
            print(
                f"   🌍 País: {cached_data.get('location', {}).get('country', 'N/A')}"
            )
        ...
        print(
            f"   💾 Tamanho atual: {stats.get('hit_count', 0) + stats.get('miss_count', 0)} operações"
        )
...
def demo_performance_summary():
    ...
    try:
        ...
        if performance_files:
            ...
            print(
                "📋 Execute 'python test_performance.py' para gerar métricas detalhadas"
            )
